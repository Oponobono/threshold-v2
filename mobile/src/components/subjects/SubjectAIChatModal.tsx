...
import { s, markdownStyles, tableStyles, PRIMARY, ASK_CLR, TXT_PRI, TXT_SEC } from '../../styles/SubjectAIChatModal.styles';
...
export const SubjectAIChatModal: React.FC<SubjectAIChatModalProps> = ({
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const errorString = `${err.message || ''} ${err.details || ''}`.toLowerCase();
  ...
  const handleUploadDocument = useCallback(async () => {
    try {
      ...
      // Procesar con Gemini
      const processResult = await processDocumentUploadHybrid(fileObj.uri, `Extrae la información completa y puntos clave de este documento para que sirva de contexto en nuestra conversación.`);

      if (!processResult?.result) {
        throw new Error(processResult?.error || t('subjects.documentProcessError', { error: 'Sin contenido extraído' }));
      }
