...
files.forEach(f => {
  try {
    ...
    // Specific fixes
    content = content.replace(/RepositoryFactory\.assessmentFiles\(\)\.getByAssessment\([^,]+,\s*([^)]+)\)/g, 'RepositoryFactory.assessmentFiles().getByField(\'assessment_id\', $1)');
    ...
    // Fix any types
    content = content.replace(/\(vid\)/g, '(vid: string)');
